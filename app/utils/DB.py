...
class DB():
    ...
    def connect(debug: bool):
        ...
        '''
        if debug:
            DB.c = MongoClient(DATABASEIPDEV, DATABASEPORT)
        else:
            DB.c = MongoClient(DATABASEIP, DATABASEPORT, username=DB_USER, password=DB_PASSWD)

    @staticmethod
    def str2ObjectId(s: str):
        return ObjectId(s)
